...
class InicioViewModel(private val inicioRepository: InicioRepository):ViewModel() {
    ...
    private val _responseSucess = MutableLiveData<BalanceBenefits>()
    val responseSucess: LiveData<BalanceBenefits> = _responseSucess
    ...
    fun onOpenHome() {
        viewModelScope.launch {
            try {
                _responseSucess.postValue(inicioRepository.saldoeBeneficios(getToken()))
            } catch (e: Exception) {
